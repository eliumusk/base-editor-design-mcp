...
def get_sgrna_translated_seq(sgrna, cds_map, abs_pos, fs, sgrna_start_pos, gene_strand, sgrna_strand, utr, e, label):
	map_key = list(abs_pos.keys())[list(abs_pos.values()).index(sgrna_start_pos)]
	sgrna_trans = {}
...
def parse_variant_df(variant_df):
	...
	parsed_variant_df.loc[:,'RefSeqID'] = parsed_variant_df.loc[:,'Name'].str.split('(', n=1).str[0]
							...
							if x == edit_nuc:
								...
								if motif_check:								
									if aa_num+'_'+str(k) in sgrna_trans.values():
										nuc_index = list(sgrna_trans.keys())[list(sgrna_trans.values()).index(aa_num+'_'+str(k))][1:]
...
def get_context_for_trans(ct_index, ct_index_check, abs_pos, cds_map, fs, sgrna_context):
	...
	i_count = 0
	cds_pos = ''
	abs_pos_rev = {v: k for k, v in abs_pos.items()}
	while ct_index < ct_index_check:
		gen_pos = abs_pos_rev.get(ct_index)
		if gen_pos is None:
			context_for_trans += 'I'
			ct_index += 1
			continue
		...
		while pos < pos_end:
			...
			if sg_str_anti == 1:
				...
				if 'I' in context_for_trans:
					map_key_context_start = list(abs_pos.keys())[list(abs_pos.values()).index(pos - 3)]
					map_key_context_end = list(abs_pos.keys())[list(abs_pos.values()).index(pos + pam_len + sg_len + 3)]
				...
				if context_for_trans != '':
					sg_gen_pos = list(abs_pos.keys())[list(abs_pos.values()).index(sgrna_end_pos)]
			...
			if sg_str_sense == 1:
				...
				if 'I' in context_for_trans:
					map_key_context_start = list(abs_pos.keys())[list(abs_pos.values()).index(pos-4)]
					map_key_context_end = list(abs_pos.keys())[list(abs_pos.values()).index(pos + pam_len + sg_len + 3)]
					if map_key_context_start in cds_map.keys():
				...
				if context_for_trans != '':
					sg_gen_pos = list(abs_pos.keys())[list(abs_pos.values()).index(sgrna_start_pos)]
...
if __name__ == '__main__':
	...
	with open(output_folder+'/sgrna_designs_'+output_name+'.txt','w') as o_sgrna, open(output_folder + '/error_report.txt', 'w') as o_error, open(output_folder+'/clinvar_annotations_'+output_name+'.txt','w') as o_clin:
		...
		for i,r in input_df.iterrows():
			...
			label = row1 if row1 != '' else row0
			print('Designing for ' + str(label))
			tr = row0
			if input_type == 'tid':
				...
				if seq_error != '':
					print(seq_error)
					w_error.writerow([row1, row1, 'N/A', 'N/A', seq_error])
					continue
				gene_name, assembly, chromosome, gene_id, gene_start, gene_end, gene_strand, tr = row1, '', '', '', 0, len(tr_seq), 1, row1
				exons, cds_sequence = get_seq_info(row0)
